...
bool isLooseElectron(unsigned int elIdx){
  ...
  if(fabs(els_etaSC().at(elIdx)) <= 1.479){
    ...
    if(fabs(els_dxyPV().at(elIdx)) >= 0.02) return false; //is this wrt the correct PV?
    if(fabs(els_dzPV().at(elIdx)) >= 0.2) return false; //is this wrt the correct PV?
    ...
    */
    ...
    if(fabs(els_dzPV().at(elIdx)) >= 0.2) return false; //is this wrt the correct PV?
    if( fabs( (1.0/els_ecalEnergy().at(elIdx)) - (els_eOverPIn().at(elIdx)/els_ecalEnergy().at(elIdx)) ) >= 0.05) return false; // |1/E - 1/p|
    if( eleRelIso03(elIdx) >= 0.15 || (eleRelIso03(elIdx) >= 0.1 && els_p4().at(elIdx).pt() < 20) ) return false; 
    if( els_conv_vtx_flag().at(elIdx) ) return false;
    if( els_lost_pixelhits().at(elIdx) > 1) return false;
    */
    //csa14 cuts
    if(fabs(els_dEtaIn().at(elIdx)) >= 0.014) return false; 
    if(fabs(els_dPhiIn().at(elIdx)) >= 0.10) return false; 
    if(els_sigmaIEtaIEta().at(elIdx) >= 0.033) return false; 
    if(els_hOverE().at(elIdx) >= 0.12) return false; 
    if(fabs(els_dxyPV().at(elIdx)) >= 0.02) return false; //is this wrt the correct PV?
    if(fabs(els_dzPV().at(elIdx)) >= 0.2) return false; //is this wrt the correct PV?
    if( fabs( (1.0/els_ecalEnergy().at(elIdx)) - (els_eOverPIn().at(elIdx)/els_ecalEnergy().at(elIdx)) ) >= 0.05) return false; // |1/E - 1/p|
    if( eleRelIso03(elIdx) >= 0.25 /*|| (eleRelIso03(elIdx) >= 0.1 && els_p4().at(elIdx).pt() < 20)*/ ) return false; 
    if( els_conv_vtx_flag().at(elIdx) ) return false;
    if( els_lost_pixelhits().at(elIdx) > 1) return false;
    //csa14 cuts
    return true;
...
bool isElectronFO(unsigned int elIdx){//fixme
  ...
  if(fabs(els_etaSC().at(elIdx)) <= 1.479){
    ...
    */
    ...
    if(els_sigmaIEtaIEta().at(elIdx) >= 0.01) return false; 
    if(els_hOverE().at(elIdx) >= 0.12) return false; 
    //FO//if(fabs(els_dxyPV().at(elIdx)) >= 0.02) return false; //is this wrt the correct PV?
    if(fabs(els_dzPV().at(elIdx)) >= 0.1) return false; //is this wrt the correct PV?
    if( fabs( (1.0/els_ecalEnergy().at(elIdx)) - (els_eOverPIn().at(elIdx)/els_ecalEnergy().at(elIdx)) ) >= 0.05) return false; // |1/E - 1/p|
    if( eleRelIso03(elIdx) >= 0.6) return false;//FO// 
    if( els_conv_vtx_flag().at(elIdx) ) return false;
    if( els_lost_pixelhits().at(elIdx) > 1) return false;
    //csa14 cuts
    return true;
    ...
    */
    ...
    if(fabs(els_dEtaIn().at(elIdx)) >= 0.012) return false; 
    if(fabs(els_dPhiIn().at(elIdx)) >= 0.03) return false; 
    if(els_sigmaIEtaIEta().at(elIdx) >= 0.031) return false; 
    if(els_hOverE().at(elIdx) >= 0.12) return false; 
    //FO//if(fabs(els_dxyPV().at(elIdx)) >= 0.02) return false; //is this wrt the correct PV?
    if(fabs(els_dzPV().at(elIdx)) >= 0.1) return false; //is this wrt the correct PV?
    if( fabs( (1.0/els_ecalEnergy().at(elIdx)) - (els_eOverPIn().at(elIdx)/els_ecalEnergy().at(elIdx)) ) >= 0.05) return false; // |1/E - 1/p|
    if( eleRelIso03(elIdx) >= 0.6 /*|| (eleRelIso03(elIdx) >= 0.1 && els_p4().at(elIdx).pt() < 20)*/ ) return false;//FO//
    if( els_conv_vtx_flag().at(elIdx) ) return false;
    if( els_lost_pixelhits().at(elIdx) > 1) return false;
    //csa14 cuts
    return true;
...
bool isMediumElectron(unsigned int elIdx){
  ...
  if(fabs(els_etaSC().at(elIdx)) <= 1.479){
    ...
    */
    ...
    if(fabs(els_dzPV().at(elIdx)) >= 0.1) return false; //is this wrt the correct PV?
    if( fabs( (1.0/els_ecalEnergy().at(elIdx)) - (els_eOverPIn().at(elIdx)/els_ecalEnergy().at(elIdx)) ) >= 0.05) return false; // |1/E - 1/p|
    if( eleRelIso03(elIdx) >= 0.21 /*|| (eleRelIso03(elIdx) >= 0.1 && els_p4().at(elIdx).pt() < 20)*/ ) return false; 
    if( els_conv_vtx_flag().at(elIdx) ) return false;
    if( els_lost_pixelhits().at(elIdx) > 1) return false;
    //csa14 cuts
    return true;
...
bool isTightElectron(unsigned int elIdx){
  ...
  if(fabs(els_etaSC().at(elIdx)) <= 1.479){
    ...
    if( els_conv_vtx_flag().at(elIdx) ) return false;
    if( els_lost_pixelhits().at(elIdx) > 0) return false;
    */
    //csa14 cuts
    if(fabs(els_dEtaIn().at(elIdx)) >= 0.009) return false; 
    if(fabs(els_dPhiIn().at(elIdx)) >= 0.03) return false; 
    ...
    if(fabs(els_dzPV().at(elIdx)) >= 0.1) return false; //is this wrt the correct PV?
    if( fabs( (1.0/els_ecalEnergy().at(elIdx)) - (els_eOverPIn().at(elIdx)/els_ecalEnergy().at(elIdx)) ) >= 0.05) return false; // |1/E - 1/p|
    if( eleRelIso03(elIdx) >= 0.1 || (eleRelIso03(elIdx) >= 0.07 && els_p4().at(elIdx).pt() < 20) ) return false; 
    if( els_conv_vtx_flag().at(elIdx) ) return false;
    if( els_lost_pixelhits().at(elIdx) > 0) return false;
    */
    //csa14 cuts
    if(fabs(els_dEtaIn().at(elIdx)) >= 0.010) return false; 
    if(fabs(els_dPhiIn().at(elIdx)) >= 0.02) return false; 
    if(els_sigmaIEtaIEta().at(elIdx) >= 0.031) return false; 
    if(els_hOverE().at(elIdx) >= 0.12) return false; 
    if(fabs(els_dxyPV().at(elIdx)) >= 0.02) return false; //is this wrt the correct PV?
    if(fabs(els_dzPV().at(elIdx)) >= 0.1) return false; //is this wrt the correct PV?
    if( fabs( (1.0/els_ecalEnergy().at(elIdx)) - (els_eOverPIn().at(elIdx)/els_ecalEnergy().at(elIdx)) ) >= 0.05) return false; // |1/E - 1/p|
    if( eleRelIso03(elIdx) >= 0.16 /*|| (eleRelIso03(elIdx) >= 0.07 && els_p4().at(elIdx).pt() < 20)*/ ) return false; 
    if( els_conv_vtx_flag().at(elIdx) ) return false;
    if( els_lost_pixelhits().at(elIdx) > 0) return false;
    //csa14 cuts
    return true;
